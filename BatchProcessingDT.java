...
public class BatchProcessingDT {
  public static void main(String[] args) throws InterruptedException {
    BatchDispatcher dispatcher = new BatchDispatcher();
    List<Thread> requestThreads = new ArrayList<>();
    List<FutureTask<String>> futures = Collections.synchronizedList(new ArrayList<>());
    
    for(int i = 1; i <= 40; i++){
      final int jobIndex = i;
      Thread requestThread = new Thread(() -> {
        try {
          FutureTask<String> future = dispatcher.addJob(jobIndex);
          futures.add(future);
